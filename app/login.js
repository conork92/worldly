...
(function() {
    ...
    // Update visibility of write forms based on login status
    function updateWriteFormsVisibility() {
        ...
        if (addQuoteBtn) addQuoteBtn.style.display = isLoggedIn ? 'block' : 'none';
        if (addQuoteForm && !isLoggedIn) {
        ...
        if (assignBtn) assignBtn.style.display = isLoggedIn ? 'block' : 'none';
        if (assignForm && !isLoggedIn) {
            assignForm.style.display = 'none';
        }
        
        // Albums edit forms are handled by click handlers, but we can disable them
        // The authenticatedFetch will throw an error if not logged in
    }
